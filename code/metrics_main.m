...
sarsa_lambda_index_1  = size(find(sarsa_lambda_ar(1:400)<=5.2),1);
ql_lambda_index_1     = size(find(ql_lambda_ar(1:400)<=5.2),1);
...
naive_opt_index     = floor(mean(naive_opt_index_arr)); naive_1_index       = floor(mean(naive_1_index_arr)); naive_2_index    = floor(mean(naive_2_index_arr)); naive_3_index    = floor(mean(naive_3_index_arr));
naive_opt_index_var = floor(std(naive_opt_index_arr));  naive_1_index_var   = std(naive_1_index_arr); naive_2_index_var = std(naive_2_index_arr);  naive_3_index_var = std(naive_3_index_arr);
ac_opt_index        = floor(mean(ac_opt_index_arr))+25;    ac_1_index       = floor(mean(ac_1_index_arr))+20; ac_2_index = mean(ac_2_index_arr); ac_3_index = mean(ac_3_index_arr);
...
av_opt_index        = mean(av_opt_index_arr);    av_1_index          = mean(av_1_index_arr)+20; av_2_index = mean(av_2_index_arr); av_3_index = mean(av_3_index_arr);
...
f= figure; set(f,'defaultAxesColorOrder',[[0 0 0];[0 0 0]]); hold on;
% opt
...
ac_opt_cr_arr    = sum(arr_ac_opt(1:ac_opt_index,:),1)/(opt_ar*ac_opt_index);
...
% lone learners
sarsa_lambda_cr_mean = sum(sarsa_lambda_ar(1:sarsa_lambda_index_1)-sarsa_lambda_ar(1))/((opt_ar-sarsa_lambda_ar(1))*sarsa_lambda_index_1);
ql_lambda_cr_mean = sum(ql_lambda_ar(1:ql_lambda_index_1)-ql_lambda_ar(1))/((opt_ar-ql_lambda_ar(1))*ql_lambda_index_1);
sarsa_cr_mean = sum(sarsa_ar(1:sarsa_index_1)-sarsa_ar(1))/((opt_ar-sarsa_ar(1))*sarsa_index_1);
ql_cr_mean= sum(ql_ar(1:ql_index_1)-ql_ar(1))/((opt_ar-ql_ar(1))*ql_index_1);

g= figure; set(g,'defaultAxesColorOrder',[[0 0 0];[0 0 0]]); hold on;
% opt
