...
while (k<max_iter && delta>stop_criterion)
    ...
    for j=1:mini_batch_size
        ...
        while(~mdp.states(state_index).terminal && lIter < max_search_iter)
            ...
            [next_action_index,next_mentor_action_index] = comply_or_defy_sig(pi_m(next_state_index),mdp.states(next_state_index).actions,temperature,next_ql,next_qd,t0/(count^(0.55)));
