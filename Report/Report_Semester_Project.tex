...
For all the learning algorithms tested, we used optimistic action-value initialisation to promote exploration. This explains why many negative rewards are perceived in the beginning. 
	...
	\section{Naive learners}
	{
		...
		{
			\paragraph{} We decided to first implement a fairly naive method. As previously discussed, the notion of compliance concerns the exploration phase - or equivalently the action selection process. 
			\newline This first naive method consists in following the teacher's recommandation with a constant probability $p$ (close from $1$), defined globally (the same for all the states). More formally, the \emph{action selection} process is defined as follows. For all $s\in\mathcal{S}$ 
			...
			\paragraph{} The learning follows the SARSA algorithm, and the usual TD(0) updates : $\forall s\in\mathcal{S}, a\in\mathcal{A}(s)$
			\begin{equation}
